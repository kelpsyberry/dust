...
impl Renderer {
    pub fn new(renderer_3d_rx: Box<dyn engine_3d::RendererRx + Send + 'static>) -> Self {
        macro_rules! buffers {
            () => {
                Buffers {
...
type FnPtrs<R> = render::FnPtrs<R, Buffers, RenderingData, Vram<R>>;

struct ThreadData {
    cur_scanline: i16,
    shared_data: Arc<SharedData>,
    fns: (FnPtrs<EngineA>, FnPtrs<EngineB>),
    renderer_3d_rx: Box<dyn engine_3d::RendererRx + Send + 'static>,
    buffers: [Buffers; 2],
}
