...
type FnPtrs<R> = render::FnPtrs<R, Buffers, Engine2d<R>, Vram>;

pub struct Renderer {
    fns: (FnPtrs<EngineA>, FnPtrs<EngineB>),
    renderer_3d_rx: Box<dyn engine_3d::RendererRx>,
    buffers: [Buffers; 2],
    framebuffer: Box<[[Scanline<u32>; SCREEN_HEIGHT]; 2]>,
}

impl Renderer {
    pub fn new(renderer_3d_rx: Box<dyn engine_3d::RendererRx>) -> Self {
        macro_rules! buffers {
            () => {
                Buffers {
        ...
        [(); R::OBJ_VRAM_LEN]: Sized,
    {
        ...
        if render_bg_obj_line {
            ...
            buffers
                ...
                .get_mut()
                .0
                .fill(BgObjPixel(backdrop as u64 | (backdrop as u64) << 32));
...
impl RendererTrait for Renderer {
    ...
    fn start_scanline(
        ...
        line: u8,
        _vcount: u8,
        _engines: (&mut Engine2d<EngineA>, &mut Engine2d<EngineB>),
        _vram: &mut Vram,
    ) {
        if line == 0 {
